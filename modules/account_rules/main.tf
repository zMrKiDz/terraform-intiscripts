...
resource tokend_account_rule "aml_alert_creator" {
...
resource tokend_account_rule "asset_updater_pro" {
  action = "create_with_tasks"
  entry_type = "reviewable_request"
  entry = {
...
output "aml_alert_creator" {
  value = "${tokend_account_rule.aml_alert_creator.id}"
}

output "asset_updater_pro" {
  value = "${tokend_account_rule.asset_updater_pro.id}"
}
